...
export default function WalletList({ address }: {address: string }) {
    //declare a state variable wallets which keeps track of WalletWithTxnCount for a given EOA address
    const [wallets, setWallets] = useState<WalletWithTxnsCount[]>([]);

    // Use useEffecet to call the fetch-wallets API endpoint and set the wallets
    useEffect(() => {
    ...
    return (
        <main className="flex flex-col justify-center items-center gap-5">
            < h1 className="text-5x1 font-bold">Your Wallets</h1>
                    ...
                    {wallets.map((wallet) => (
                        ...
                        >
                            ...
                            <div className="flex flex-col py-2 divide-y divide-gray-600">
                                {wallet.signers.length > 0 && wallet.signers.map((signer, idx) => (
                                    <div
